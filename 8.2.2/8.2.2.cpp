...
int main() {
    ...
    auto limit_db = OneShotDB(new VeryHeavyDatabase, 2);
    std::cout << limit_db.GetData("key") << std::endl;
    std::cout << limit_db.GetData("key") << std::endl;
    std::cout << limit_db.GetData("key") << std::endl;
    std::cout << limit_db.GetData("key") << std::endl;
     
    auto cached_db = CacheProxyDB(new VeryHeavyDatabase);
    auto test_db = TestDB(new VeryHeavyDatabase);
